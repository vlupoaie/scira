...
function chooseRightOperators() {
    var this_form = document.getElementById('test_plugin_form');
    var all_form_divs = this_form.getElementsByTagName('div');
    var all_form_groups = [];
    ...
    var operator_value;
    for (i = 1; i < all_form_groups.length; i++) {
...
function appendLinesTest(object, timestamp) {
    ...
    $.getJSON("more_test_table_lines", {
        last_timestamp: timestamp,
        start_slice: parseInt(count_label.innerText)
    }).done(function (data) {
        ...
        object.setAttribute('onclick', 'appendLinesTest(this, ' + data['new_timestamp'] + ')');
        $("table").trigger("update");
        count_label = object.parentNode.nextElementSibling.nextElementSibling.getElementsByTagName('h4')[0]
            .getElementsByTagName('label')[1];
